...
var _Sources = (() => {
  ...
  var parseMangaDetails = ($2, mangaId) => {
    const titles = [];
    titles.push((0, import_html_entities.decode)($2("img", "div.fixed-img").attr("alt")?.trim() ?? ""));
    const altTitles = $2("h2.alternative-title.text1row", "div.main-head").text().trim().split(",");
    for (const title of altTitles) {
      titles.push((0, import_html_entities.decode)(title));
    }
  ...
  var parseHomeSections = ($2, sectionCallback) => {
    ...
    for (const manga of $2("li", "div#recommend-novel-slider").toArray()) {
      ...
      mostViewedSection_Array.push(App.createPartialSourceManga({
        ...
        mangaId: id,
        subtitle: (0, import_html_entities.decode)(subtitle) + " Stars"
      }));
    }
    ...
    for (const manga of $2("li.novel-item", "ul.novel-list").toArray()) {
      ...
      updateSection_Array.push(App.createPartialSourceManga({
        ...
        title: (0, import_html_entities.decode)(title),
        mangaId: id,
        subtitle: (0, import_html_entities.decode)(subtitle + " ago")
      }));
    }
