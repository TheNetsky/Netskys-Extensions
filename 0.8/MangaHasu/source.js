...
var _Sources = (() => {
  ...
  var parseMangaDetails = ($2, mangaId) => {
    const titles = [];
    titles.push((0, import_html_entities.decode)($2("div.info-title > h1").text()?.trim() ?? ""));
    ...
    const description = (0, import_html_entities.decode)($2("div.content-info > div").first().text().trim() ?? "");
  ...
  var parseChapters = ($2, mangaId, baseURL) => {
    ...
    for (const chapter of $2("tr", "div.list-chapter").toArray()) {
      const title = (0, import_html_entities.decode)($2("td.name > a", chapter).children().remove().end().text().trim());
