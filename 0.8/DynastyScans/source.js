...
var _Sources = (() => {
  ...
  var DynastyScans = class {
    ...
    async getMangaDetails(mangaId) {
      ...
      const data2 = typeof response.data === "string" ? JSON.parse(response.data) : response.data;
      const author = data2.tags.filter((x) => x.type == "Author");
      const status = data2.tags.find((x) => x.type == "Status");
      const arrayTags = [];
      for (const tag of data2.tags.filter((x) => x.type == "General")) {
      ...
      if (data2.description) {
        const $2 = load(`<div>${data2.description}</div>`);
        description = $2("div").text().trim();
      } else {
        description = `Tags: ${data2.tags.map((x) => x.name).join(", ")}`;
      }
      return App.createSourceManga({
        id: mangaId,
        mangaInfo: App.createMangaInfo({
          titles: [...[data2.name], ...data2.aliases],
          image: await this.getOrSetThumbnail("SET", mangaId, DS_DOMAIN + data2.cover),
    ...
    async getChapters(mangaId) {
      ...
      const response = await this.requestManager.schedule(request, 1);
      const data2 = typeof response.data === "string" ? JSON.parse(response.data) : response.data;
      const seriesData = data2;
      const doujinData = data2;
      const chapters = [];
    ...
    async getChapterDetails(mangaId, chapterId) {
      ...
      const data2 = typeof response.data === "string" ? JSON.parse(response.data) : response.data;
      const images = [];
      for (const image of data2.pages) {
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      metadata = data2.current_page < data2.total_pages ? { page: page + 1 } : void 0;
    ...
    async getSearchTags() {
      ...
      while (getMore) {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const data2 = typeof response.data === "string" ? JSON.parse(response.data) : response.data;
        for (const tagType of data2.tags) {
        ...
        if (data2.current_page >= data2.total_pages) {
          getMore = false;
        }
      }
      return tagSections;
    ...
    async getSearchResults(query, metadata) {
      async function getTagId(global, tags) {
        ...
        for (const tag of tags) {
          ...
          const data2 = typeof response2.data === "string" ? JSON.parse(response2.data) : response2.data;
          const tagId = data2.find((x) => x.type == "General");
      ...
      const results = [];
      for (const item of $2("dl.chapter-list > dd").toArray()) {
        const id = $2("a.name", item).attr("href")?.replace(/\/$/, "") ?? "";
        const title = $2("a.name", item).text().trim() ?? "";
        const tags = $2("span.tags > a", item).toArray().map((x) => $2(x).text().trim()).join(", ");
    ...
    async getOrSetThumbnail(method, mangaId, coverURL) {
      async function fetchThumbnail(global) {
        try {
          ...
          const data2 = typeof response.data === "string" ? JSON.parse(response.data) : response.data;
          return data2.cover ? DS_DOMAIN + data2.cover : "";
        } catch (error) {
